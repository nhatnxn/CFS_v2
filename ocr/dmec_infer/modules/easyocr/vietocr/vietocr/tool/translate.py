...
def translate(img, model, max_seq_length=128, sos_token=1, eos_token=2, get_prob=False):
    ...
    with torch.no_grad():
        ...
        if torch.cuda.is_available():
            translated_sentence = torch.cuda.LongTensor([[sos_token] * len(img)], device=device)
            char_probs = torch.cuda.LongTensor([[1] * len(img)])
        else:
            translated_sentence = torch.LongTensor([[sos_token] * len(img)], device=device)
            char_probs = torch.LongTensor([[1] * len(img)])
